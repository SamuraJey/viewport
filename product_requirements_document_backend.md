...
* Загрузка фотографий (ограничение 10 МБ на файл).
...
| US2 | Фотограф | Как фотограф, я хочу загрузить фотографии (≤ 10 МБ), чтобы собрать галерею.                  | Успешная загрузка в S3; в БД — запись с URL файла.     |
...
* **Безопасность:**
  ...
  * Ограничение размера загрузок (10 МБ).
...
| POST  | `/galleries/{id}/photos`            | Загрузить фото (≤ 10 МБ) | Bearer JWT |
