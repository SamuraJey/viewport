...
- **Pages**: In `frontend/src/pages/`, use custom hooks for pagination/selection/modals instead of manual state (see DashboardPage.tsx, GalleryPage.tsx for examples).
  ...
  - `PublicGalleryPage.tsx` should render a loading skeleton (`components/public-gallery/SkeletonPhotoGrid.tsx`) instead of a blank screen while shared photos are loading.
  - Public masonry cards use the dedicated `MasonryPhotoCard` component (not `LazyImage`) so masonry sizing is controlled by grid-row spans plus card-level aspect-ratio fallback.
