...
- Repositories:
  ...
  - Share links may be password-protected via `share_links.password_hash`; expose only `has_password` in APIs. Share-link passwords must stay within bcrypt's 72 UTF-8 byte input limit. Public `/s/{share_id}` routes must use the shared password gate and preserve `404` for missing/inactive links and `410` for expired links.
