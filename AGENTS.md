...
- **Pages**: In `frontend/src/pages/`, use custom hooks for pagination/selection/modals instead of manual state (see DashboardPage.tsx, GalleryPage.tsx for examples).
  ...
  - Folder and directory upload is a frontend-only intake path. The gallery header **Add photos** control is a split action: the main button opens the regular file picker (one click), the adjacent chevron opens a popover with **Upload files** and **Upload folder**. The folder option uses a hidden `<input type="file" webkitdirectory>` and is feature-detected — on unsupported browsers (Safari) the folder action is hidden. Dropping a directory onto the gallery page or the open review modal recursively flattens all nested files into a flat queue via `extractFilesFromEvent` in `components/upload/uploadUtils.ts`. The full recursive traversal runs **only on the actual drop** (`getFilesFromEvent` returns raw `DataTransferItem`s on `dragenter` so the overlay can classify the payload without reading the tree); `UploadDropzone` maps the Entry-API traversal to `isProcessing` and surfaces a controlled `DirectoryReadError` through `onError`. Directory structure is never preserved: the relative path is stored in a `WeakMap` (never on the `File`) and used only for deduplication identity (`getUploadFileKey` includes the normalized source path); it is never sent to the backend. Same basename from different subdirectories stays distinct; re-adding the same file from the same source path deduplicates. `createReader().readEntries()` is called repeatedly until empty (Chromium caps ~100 per call), yields to the event loop between batches, and imposes no file-count cap. A visible scanning state (`isProcessing` / `isScanningDrop`) blocks second intake during traversal. Source path is transferred to the resized `File` via `transferUploadSourcePath` after image resize. Backend, S3, quota, presign, and multipart contracts are unchanged.
  - The Project page empty state ("Build this project with galleries") also offers **Upload folder**: it creates a new gallery named after the chosen folder (truncated to the gallery-name limit) and opens it with the folder's supported files staged. Because the gallery does not exist until created, `ProjectPage` enqueues the selected files in the in-memory `pendingFilesQueue` (`frontend/src/lib/pendingFilesQueue.ts`) keyed by the new gallery id, then navigates to the gallery route; `GalleryPage` consumes the queue once its uploader is ready and feeds the files through the existing `handleExternalFiles` review flow. Files are held in memory only (never serialized or sent), the queue is cleared on first read (idempotent under StrictMode), and the folder action is feature-detected via `webkitdirectory`. No backend change is required.
