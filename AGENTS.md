...
- Repositories:
  ...
  - `POST /projects` is now the canonical creation flow and must create both the project and its initial gallery. Legacy top-level gallery creation remains compatibility-only and should auto-wrap into a new one-gallery project.
...
- **Pages**: In `frontend/src/pages/`, use custom hooks for pagination/selection/modals instead of manual state (see DashboardPage.tsx, GalleryPage.tsx for examples).
  - Dashboard is project-only and should navigate directly into each project's entry gallery instead of showing separate standalone gallery cards.
