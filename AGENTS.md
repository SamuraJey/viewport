...
- Repositories:
  ...
  - Gallery/project listing must preserve the split between standalone galleries and project galleries; owner dashboard gallery queries should use server-side filters rather than client-side partitioning.
  ...
  - Public share access dispatches by scope: gallery links return gallery photos, project links open listed project galleries, inactive links must remain non-disclosing (`404`), and expired links return `410` so frontend can render a dedicated expiration state.
...
- **Pages**: In `frontend/src/pages/`, use custom hooks for pagination/selection/modals instead of manual state (see DashboardPage.tsx, GalleryPage.tsx for examples).
  - Dashboard now shows **Projects** and **Standalone galleries**. Standalone gallery listing still uses server-side `search`, `sort_by`, `order`, `page`, and `size` query params; avoid client-side full-dataset fetch loops on initial load/refresh.
  - `ProjectPage.tsx` is the owner surface for project metadata, gallery visibility/order, and project-scoped share links; project galleries should be presented as a horizontal list of names rather than preview cards.
  ...
  - Public gallery sorting is not user-adjustable in `PublicGalleryPage.tsx`; photographer-configured settings are edited in private gallery and persisted on the gallery model. For `scope_type = "project"`, the public page should open the first listed gallery by default and render a horizontal list of gallery names without preview cards.
