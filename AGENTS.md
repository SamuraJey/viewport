...
- **Pages**: In `frontend/src/pages/`, use custom hooks for pagination/selection/modals instead of manual state (see DashboardPage.tsx, GalleryPage.tsx for examples).
  ...
  - Gallery upload intake is centralized in `components/gallery/GalleryDropZone.tsx` and `components/PhotoUploader.tsx`: page-wide `react-dropzone` drag state, clipboard files outside editable controls, deduplication, and a 200-file queue all feed `components/upload/UploadConfirmModal.tsx`, which stays mounted across gallery tabs. Preserve the existing presigned image and multipart video APIs; resolve missing browser MIME types from supported extensions, never reuse failed photo/upload intents on retry, lazy-load queue thumbnails near the viewport, carry per-file UI progress through `PhotoUploadProgress.files`, and keep queue order on the accessible dnd-kit grip before upload starts. See `docs/photo-upload-ux.md` for the complete interaction and cancellation contract.
