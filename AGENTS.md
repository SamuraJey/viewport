...
- Repositories:
  ...
  - `Project` is the parent container and `Gallery` is still the leaf upload unit; a gallery can be standalone (`project_id = null`) or belong to a project.
  ...
  - Gallery/project listing must preserve the split between standalone folders and project folders; owner dashboard folder queries should use server-side filters rather than client-side partitioning.
  - `galleries.project_visibility` controls project-share visibility: `listed` folders appear inside project shares, `direct_only` folders are hidden from project shares and remain accessible only via their own direct gallery share link.
  ...
  - Selection/favorites is gallery-scope only; do not enable or fake selection flows for project-scoped share links.
  ...
  - Public share access dispatches by scope: folder links return gallery photos, project links return listed folders, inactive links must remain non-disclosing (`404`), and expired links return `410` so frontend can render a dedicated expiration state.
...
- **Pages**: In `frontend/src/pages/`, use custom hooks for pagination/selection/modals instead of manual state (see DashboardPage.tsx, GalleryPage.tsx for examples).
  - Dashboard now shows **Projects** and **Standalone folders**. Standalone folder listing still uses server-side `search`, `sort_by`, `order`, `page`, and `size` query params; avoid client-side full-dataset fetch loops on initial load/refresh.
  - `ProjectPage.tsx` is the owner surface for project metadata, folder visibility/order, and project-scoped share links.
  ...
  - Public gallery sorting is not user-adjustable in `PublicGalleryPage.tsx`; photographer-configured settings are edited in private gallery and persisted on the gallery model. The same page also renders project-share folder listings when `scope_type = "project"`.
