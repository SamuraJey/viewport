...
import type {
    PublicGridDensity,
    PublicGridLayout,
} from '../../hooks/usePublicGalleryGrid';
...
export const PublicGalleryPhotoSection = ({
    ...
    return (
        <div className="bg-surface-foreground/5 rounded-2xl p-6 border border-border" {...touchHandlers} style={{ touchAction: 'pan-y' }}>
            ...
            {photos.length > 0 ? (
                <>
                    ...
                    {!hasMore && photos.length > 50 && (
                        <div className="text-center py-8 text-muted text-sm">All photos loaded ({photos.length} total)</div>
                    )}
                </>
            ) : (
