...
import { AppearanceInfoTooltip } from './AppearanceInfoTooltip';

import type { SaveStatus, AppearanceDraft, AppearanceEditorProps } from './constants';
import {
  SAVE_STATUS_LABELS,
  AUTOSAVE_DEBOUNCE_MS,
  MAX_PREVIEW_PHOTOS,
  COVER_PICKER_PAGE_SIZE,
  DISPLAY_OPTION_CONFIG,
} from './constants';
import { formatPublicGalleryDate, clampFocal } from './utils';
...
export const AppearanceEditor = ({
      ...
      debounceRef.current = setTimeout(() => {
        ...
        saveQueueRef.current = saveQueueRef.current
          ...
          .then(async () => {
            ...
            try {
              const synced = await onSaveAppearance({
  ...
  const previewHeroDate = formatPublicGalleryDate(previewDate);
    ...
    <div className="grid min-h-[72vh] overflow-hidden border border-border/30 bg-surface shadow-xs dark:border-border/20 dark:bg-surface-dark lg:grid-cols-[350px_minmax(0,1fr)]">
      <aside className="space-y-8 bg-surface px-6 py-8 dark:bg-surface-dark lg:max-h-[calc(100vh-9rem)] lg:overflow-y-auto lg:px-8">
        <div className="flex items-center gap-2">
          ...
          <AppearanceInfoTooltip text={infoTooltip} />
        </div>

        {/* ---- Save status ---- */}
        {saveStatus !== 'idle' && (
          <div aria-live="polite" className="rounded-xl px-4 py-2.5 text-sm font-medium">
            {saveStatus === 'saving' ? (
