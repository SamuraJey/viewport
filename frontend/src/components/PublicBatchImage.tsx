...
export const PublicBatchImageProvider = ({
  shareId,
  children,
}: {
  shareId: string;
  ...
  return <BatchContext.Provider value={{ shareId, loadBatch }}>{children}</BatchContext.Provider>;
};
...
export const PublicBatchImage = ({
  ...
  useEffect(() => {
    ...
    const loadImage = async () => {
      ...
      try {
        ...
        const respWithDims = response as unknown as {
          width?: number | null;
          height?: number | null;
        };
        if (respWithDims.width && respWithDims.height) {
          const w = respWithDims.width;
          const h = respWithDims.height;
          const ratio = (w || 1) / (h || 1);
          if (ratio > 1.6) setAspectClass('photo--wide');
          else if (ratio < 0.7) setAspectClass('photo--tall');
          else setAspectClass('photo--square');
  ...
  }, [shareId, photoId, batchContext, loading]);

  if (isLoading) {
    return (
      <div
        className={`bg-surface-foreground dark:bg-surface animate-pulse flex items-center justify-center ${className}`}
      >
  ...
  if (error) {
    return (
      <div
        className={`bg-red-100 text-red-500 text-sm p-2 flex items-center justify-center ${className}`}
      >
        {error}
      </div>
    );
