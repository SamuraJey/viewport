...
def proxy_file(key: str) -> StreamingResponse:
    ...
    logger.info(f"[proxy_file] Received request for key: {key}")
    ...
    try:
        ...
        logger.info(f"[proxy_file] Successfully fetched object from S3: bucket={bucket}, key={key}")
    except Exception as e:
        logger.error(f"[proxy_file] Error fetching object: {e}")
    ...
    logger.info(f"[proxy_file] Returning StreamingResponse with media_type={mime_type}")
